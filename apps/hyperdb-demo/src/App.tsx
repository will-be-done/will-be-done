...
import { useDispatch, useSyncSelector } from "@will-be-done/hyperdb-lib/react";
import {
  clearWorkload,
  generateWorkload,
...
type LastRun = {
  label: string;
  durationMs: number;
  result: WorkloadResult | { projectsDeleted: number; tasksDeleted: number };
};
...
function App() {
  ...
  const dashboard = useSyncSelector(
    () => getDashboardSnapshot(taskLimit, projectLimit, selectedProjectId),
    [taskLimit, projectLimit, selectedProjectId],
  );
  ...
  const runMeasured = (
    ...
    workload: () =>
      | WorkloadResult
      | { projectsDeleted: number; tasksDeleted: number },
  ) => {
    setIsWorking(true);
  ...
  const runTenThousandTasks = () => {
    runMeasured("10,000 task batch", () => dispatch(generateWorkload(20, 500)));
  };
  ...
  return (
    <main className="app-shell">
      <header className="topbar">
        <div>
          <p className="eyebrow">HyperDB Demo</p>
      ...
      <section className="controls-band" aria-label="Workload controls">
        ...
        <div className="button-group">
          ...
          <button
            ...
            className="danger"
            onClick={clearAll}
            disabled={isWorking || dashboard.totalTasks + dashboard.totalProjects === 0}
          >
            Clear
          </button>
        </div>
            ...
            {dashboard.projects.length === 0 ? (
              ...
              dashboard.projects.map((project) => (
                ...
                >
                  ...
                  <span>
                    {formatNumber(dashboard.projectTaskCountsById[project.id] ?? 0)}{" "}
                    tasks
                  </span>
                </button>
              ))
