TODO: 

1. Fix suggestion from codex
1. after callback should accept same queries
1. remove runQuery(). Just yield*(maybe?)
1. start index with by...
1. Move code by files/dirs. Follow convention
1. Check if code has race condition. Maybe selector.ts has race condition? And subacriable-db.ts
1. Ask if trai apporach overall good. Passing data with tree
1. Generate demo and check porfile on bit inserts/selects
1. do we need cache db?
1. optimize SubscribableDBTx
1. Think about isert if record exists rule
1. Generate docs
1. play with efffect-ts
1. don't remember but I had problems with default ids. But not sure
1. astnc requests for async drivers
1. check that utf8 sorting of string is same in js
1. fix update() to not work if record not exists
1. dev tool. Check tinybase. Check powersync

2 DONE. take/order/first
4. remove runQuery(). Just yield*(maybe?)
5. after callback should accept same queries

Then:
1. Nested index
2. When querying index - keep same order
3. Index name should start by...

Maybe:
1. filter
