...
describe("CachedDB", async () => {
  ...
  it("insert/upsert/delete write to cache immediately and defer primary", async () => {
    ...
    expect(
      cache.intervalScan(tasksTable, "byId", [
        { eq: [{ col: "id", val: "1" }] },
      ]),
    ).toEqual([task]);
    ...
    expect(
      primary.intervalScan(tasksTable, "byId", [
        { eq: [{ col: "id", val: "1" }] },
      ]),
    ).toEqual([]);
  ...
  it("hash lookup after btree full scan — serves from cache, no primary hit", async () => {
    ...
    const result = db.intervalScan(tasksTable, "byId", [
      { eq: [{ col: "id", val: "2" }] },
    ]);
  ...
  it("hash lookup caches individual keys — second lookup serves from cache", async () => {
    ...
    const result1 = db.intervalScan(tasksTable, "byId", [
      { eq: [{ col: "id", val: "1" }] },
    ]);
    ...
    const result2 = db.intervalScan(tasksTable, "byId", [
      { eq: [{ col: "id", val: "1" }] },
    ]);
    ...
    primaryScanSpy.mockClear();
    const result3 = db.intervalScan(tasksTable, "byId", [
      { eq: [{ col: "id", val: "2" }] },
    ]);
  ...
  it("hash key tracking in transactions — propagates on commit", async () => {
    ...
    const tx = db.beginTx();
    const txResult = tx.intervalScan(tasksTable, "byId", [
      { eq: [{ col: "id", val: "1" }] },
    ]);
    expect(txResult).toEqual(tasks);
    expect(primaryScanSpy).toHaveBeenCalledTimes(1);
