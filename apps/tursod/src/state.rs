...
fn is_rollback_statement(sql: &str) -> bool {
  ...
  match words.next() {
    None => true,
    Some(transaction) if transaction.eq_ignore_ascii_case("TRANSACTION") => words.next().is_none(),
    Some(_) => false,
  }
}
...
impl StatementObservation {
  fn new(
    ...
    Self {
      ...
      transaction_finished: autocommit_before == Some(false) && autocommit_after == Some(true),
    }
  }
    ...
    async {
      ...
      let result = match result {
        ...
        Err(error) => {
          ...
          observation.transaction_finished =
            autocommit_after == Some(false) && observation.autocommit_after == Some(true);
          observation.log_failure(&error, &rollback, self.pressure_snapshot());
          Err(StatementExecutionError {
            error,
            autocommit_after: observation.autocommit_after,
            poison_connection: !rollback.connection_is_usable(observation.autocommit_after),
          })
        }
      };
  ...
  pub(crate) async fn open_conn(
    ...
    let opened_db = cell
      .get_or_try_init(|| async {
        ...
        let repair = tokio::task::spawn_blocking(move || repair_malformed_indexes(&repair_path))
          .await
          .map_err(TursodError::internal)?
          .map_err(TursodError::internal)?;
...

    {
      ...
      state
        .exec_stmts(
          ...
          TransactionState::Autocommit,
          vec![
            statement("CREATE TABLE users (id TEXT PRIMARY KEY, idx_byIds_sort_key TEXT)"),
            statement(
