...
export const getBackups = (state: RootState): Backup => {
  ...
  for (const task of Object.values(state.task.byIds)) {
    tasks.push({
      ...
      templateId: task.templateData?.templateId || null,
      templateDate: task.templateData?.templateDate || null,
    });
  }
...
export const loadBackups = (backup: Backup) => {
  ...
  for (const taskBackup of backup.tasks) {
    ...
    const task: Task = {
      ...
      templateId: taskBackup.templateId,
      templateDate: taskBackup.templateDate,
    };
