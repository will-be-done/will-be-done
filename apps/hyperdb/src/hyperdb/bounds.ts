  ...
  return where.map((clause) => {
    ...
    for (const condition of eq) {
      ...
      if (colConditions.eq !== undefined) {
        throw new Error(`Multiple equality conditions for column '${condition.col}'`);
      }
    ...
    for (const [col, conditions] of conditionsByCol) {
      if (conditions.eq !== undefined && (conditions.gte !== undefined || conditions.gt !== undefined || conditions.lte !== undefined || conditions.lt !== undefined)) {
    ...
    for (let i = 0; i < indexCols.length; i++) {
      ...
      // If this is a range condition, we cannot use further columns
    ...
    for (const col of conditionsByCol.keys()) {
      ...
      if (colIndex >= usablePrefix) {
        throw new Error(`Cannot use column '${col}' without specifying conditions for all preceding columns`);
      }
    }
