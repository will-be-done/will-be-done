...
export class HyperDBSQLite {
  ...
  private createTable<T extends TableSchema>(
  ...
  private createIndexes<T extends TableSchema>(
    tableDef: TableDefinition<T>,
  ): void {
  ...
  private buildWhereClause(
    ...
    const indexColumns =
      indexName === "ids" ? ["id"] : tableDef.indexes[indexName];
    if (!indexColumns) {
    ...
    if (selectResult.length > 0 && selectResult[0].values) {
      for (const row of selectResult[0].values) {
        ...
        if (predicate(record)) {
          ...
          const updateResult = this.db.exec(updateSQL, [
