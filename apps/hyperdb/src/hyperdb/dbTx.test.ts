  ...
  for (const driver of [
    ...
    describe(`${driver.constructor.name}`, () => {
      ...
      it("transaction with btree range queries", async () => {
        ...
        expect(rangeResult.map(t => t.title).sort()).toEqual(["Banana", "Cherry"]);
        ...
        expect(postCommitResult.map(t => t.title).sort()).toEqual(["Banana", "Cherry"]);
      });
      ...
      it("hash index equality queries in transactions", async () => {
        ...
        expect(sameTitleResults.map(t => t.id).sort()).toEqual(["task-1", "task-2"]);
      ...
      it("btree index range queries in transactions", async () => {
        ...
        expect(rangeResults.map(t => t.title).sort()).toEqual(["Banana", "Cherry"]);
        ...
        expect(greaterResults.map(t => t.title).sort()).toEqual(["Date", "Elderberry"]);
      ...
      it("transaction with mixed operations on different indexes", async () => {
        ...
        expect(rangeResult.length).toBe(2);
        expect(rangeResult.map(t => t.title).sort()).toEqual(["Alpha-Updated", "Gamma"]);
        ...
        expect(finalTasks.map(t => t.id).sort()).toEqual(["task-1", "task-3"]);
      });
      ...
      it("complex query patterns - multiple where clauses", async () => {
        ...
        expect(multiClauseResults.map(t => t.title).sort()).toEqual(["Alpha", "Gamma"]);
      ...
      it("edge case - boundary values in btree queries", async () => {
        ...
        expect(upperBoundary.map(t => t.title).sort()).toEqual(["Z", "ZZZZZ"]);
      ...
      it("hash index with multiple records having same value", async () => {
        ...
        expect(duplicateResults.map(t => t.id).sort()).toEqual(["task-1", "task-2", "task-3"]);
      ...
      it("hash index consistency between transaction and direct operations", async () => {
        ...
        expect(txResults.map(t => t.id).sort()).toEqual(["direct-1", "direct-2", "tx-1"]);
        ...
        expect(directResults.map(t => t.id).sort()).toEqual(["direct-1", "direct-2"]);
      ...
      it("hash index rollback with multiple values", async () => {
        ...
        expect(finalResults.length).toBe(3);
        expect(finalResults.map(t => t.id).sort()).toEqual(["keep-1", "keep-2", "keep-3"]);
      ...
      it("hash index mixed operations preserving value groups", async () => {
        ...
        expect(alphaResults.map(t => t.id).sort()).toEqual(["a1", "a3", "g1"]);
